...
#
# Set ELYXION_VERSION to pin a specific release (e.g. v1.3.0).
...
ELYXION_VERSION="${ELYXION_VERSION:-v1.3.0}"
...
if command -v curl >/dev/null 2>&1; then
  if ! curl -fL --progress-bar -o "$TMPDIR/elyxion.tar.gz" "$DOWNLOAD_URL" 2>&1; then
    ...
    if ! curl -fL -o "$TMPDIR/elyxion.tar.gz" "$DOWNLOAD_URL" 2>&1; then
      ...
      err "  ELYXION_VERSION=v1.3.0 ./install.sh"
      echo ""; echo "Log saved to: $ELYXION_LOG"
      rm -rf "$TMPDIR"
      exit 1
    fi
...
if [ "$DOWNLOAD_SIZE" -lt 1024 ]; then
  ...
  err "Try pinning a known-good version: ELYXION_VERSION=v1.3.0 ./install.sh"
  echo ""; echo "Log saved to: $ELYXION_LOG"
  rm -rf "$TMPDIR"
  exit 1
fi
