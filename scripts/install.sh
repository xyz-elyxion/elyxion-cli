...
#
# Set ELYXION_VERSION to pin a specific release (e.g. v1.2.2).
...
# When cutting a release, update ELYXION_VERSION below.

set -euo pipefail

# ---- Configuration ------------------------------------------------
ELYXION_REPO="${ELYXION_REPO:-xyz-elyxion/elyxion-cli}"
ELYXION_VERSION="${ELYXION_VERSION:-v1.2.2}"
...
if command -v curl >/dev/null 2>&1; then
  if ! curl -fL --progress-bar -o "$TMPDIR/elyxion.tar.gz" "$DOWNLOAD_URL" 2>&1; then
    ...
    if ! curl -fL -o "$TMPDIR/elyxion.tar.gz" "$DOWNLOAD_URL" 2>&1; then
      ...
      err "  ELYXION_VERSION=v1.2.2 ./install.sh"
      echo ""; echo "Log saved to: $ELYXION_LOG"
      rm -rf "$TMPDIR"
      exit 1
    fi
...
if [ "$DOWNLOAD_SIZE" -lt 1024 ]; then
  ...
  err "Try pinning a known-good version: ELYXION_VERSION=v1.2.2 ./install.sh"
  echo ""; echo "Log saved to: $ELYXION_LOG"
  rm -rf "$TMPDIR"
  exit 1
fi
