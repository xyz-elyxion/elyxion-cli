...
class Installer {
  constructor(options = {}) {
    ...
    this.lockFilePath = path.join(this.cwd, PACKAGE_LOCK);
  }
  ...
  async installPackage(name, version) {
    ...
    const pkgDir = path.join(this.nodeModulesPath, name);
    ...
    if (pkg.dependencies && Object.keys(pkg.dependencies).length > 0) {
      for (const [depName, depVersion] of Object.entries(pkg.dependencies)) {
        const depDir = path.join(this.nodeModulesPath, depName);
  ...
  async createLockFile(installed) {
    ...
    for (const pkg of installed) {
      const pkgDir = path.join(this.nodeModulesPath, pkg.name);
