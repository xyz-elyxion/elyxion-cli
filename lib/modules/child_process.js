...
function execSync(command, options) {
  ...
  // Native exec — runs the command synchronously via /bin/sh (or cmd.exe)
  // and captures stdout/stderr.
  if (typeof globalThis.__elyxion_exec === 'function') {
    const result = globalThis.__elyxion_exec(String(command));
    if (result === undefined) {
      if (options.encoding && options.encoding !== 'buffer') return '';
      return Buffer.alloc(0);
    }
    const stdout = Buffer.from(result.stdout || '', 'utf-8');
    if (options.encoding && options.encoding !== 'buffer') {
      return stdout.toString(options.encoding);
    }
    return stdout;
  }
