...
function normalize(path) {
  ...
  // Normalize the path
  path = normalizePathSegments(path, !isPathAbsolute);
  if (isPathAbsolute) path = '/' + path;
...
function extname(path) {
  ...
  const extensionDot = baseName.lastIndexOf('.');
  if (extensionDot > 0) return baseName.substring(extensionDot);
