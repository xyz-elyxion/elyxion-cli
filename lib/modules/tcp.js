...
module.exports = {
  ...
  createConnection: function (options, callback) {
    ...
    var callbacks = {
      ...
      end: function () { socket.emit('end'); socket.emit('close'); },
      error: function (message) { var err = new Error(message); socket.emit('error', err); }
    };
    if (typeof globalThis.__elyxion_tcp_connect !== 'function') {
      process.nextTick(function () {
        socket.emit('error', new Error('TCP client support is unavailable in this Elyxion runtime'));
      });
      return socket;
    }
    var id = globalThis.__elyxion_tcp_connect(host, port, callbacks);
    socket._id = id;
    return socket;
  }
};