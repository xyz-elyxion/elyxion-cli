...
class ClientRequest extends stream.Writable {
  ...
  _connect() {
    const secure = this.protocol === 'https:' || String(this.port) === '443' || this.secure === true;
    const connectFn = secure
      ? (tls && typeof tls.connect === 'function' ? tls.connect : net.createConnection)
...
class ServerResponse extends stream.Writable {
  ...
  write(data, encoding, callback) {
    ...
    // Send the body chunk to the client socket
    if (this.socket && data !== undefined && data !== null) {
      this.socket.write(data.toString ? data.toString() : String(data));
    }
    ...
    // Connection: close — flush the response and close the socket
    if (this.socket) {
      this.socket.end();
    }
  ...
  _handleConnection(socket) {
    ...
    socket.on('data', (data) => {
      ...
      if (!headersComplete) {
        ...
        // ServerResponse writes to the underlying socket
