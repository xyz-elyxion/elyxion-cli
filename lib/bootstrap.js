...
(function() {
  ...
  // Keep the CommonJS-compatible global alias available to built-in modules.
  if (typeof globalObj.global === 'undefined') globalObj.global = globalObj;
  ...
  // Upgrade the native process object to an EventEmitter so that
  // process.on('SIGINT', ...) and other process event APIs work.
  // (EventEmitter is a class, so wire up the prototype and state manually
  // rather than invoking its constructor.)
  ...
  // setTimeout/setInterval/clearTimeout/clearInterval are registered natively
  // by SetupNativeFunctions (uv_timer-backed) before this script runs, so the
  // guards below only fire if the host runtime omits them. setImmediate is not
  // yet native-backed and keeps a placeholder until it is wired up.
  ...
  // ---- Buffer helpers (latin1 / base64 / hex) ----------------
  ...
  function base64Decode(str) {
    ...
    for (let i = 0; i < clean.length; i++) {
      if (clean[i] === '=') break;
      buffer = (buffer << 6) | B64C.indexOf(clean[i]);
      bits += 6;
      if (bits >= 8) {
        bits -= 8;
  ...
  // Proxy numeric index access onto the backing Uint8Array so code can do
  // buffer[i] like Node's Buffer, while keeping all methods on the target.
  const BufferIndexHandler = {
    get(target, prop) {
      if (typeof prop !== 'symbol' && String(Number(prop)) === prop) {
    ...
    set(target, prop, value) {
      if (typeof prop !== 'symbol' && String(Number(prop)) === prop) {
        target._data[Number(prop)] = value & 0xff;
        return true;
      }
  ...
  class Buffer {
    constructor(arg, encodingOrOffset, length) {
      ...
      // Support Node-style numeric index access (buffer[0]) via a Proxy.
      return new Proxy(this, BufferIndexHandler);
    }
    ...
    static from(data, encodingOrOffset, length) {
      if (typeof data === 'string') {
        const encoding = String(encodingOrOffset || 'utf-8').toLowerCase();
        if (encoding === 'latin1' || encoding === 'binary' || encoding === 'ascii') {
          const buf = new Buffer(data.length);
          for (let i = 0; i < data.length; i++) buf._data[i] = data.charCodeAt(i) & 0xff;
          return buf;
        }
        if (encoding === 'base64') {
    ...
    readUInt8(offset = 0) { return this._data[offset]; }
    readUInt16BE(offset = 0) { return (this._data[offset] << 8) | this._data[offset + 1]; }
    readUInt16LE(offset = 0) { return (this._data[offset + 1] << 8) | this._data[offset]; }
    readUInt32BE(offset = 0) {
      return (this._data[offset] * 0x1000000) +
        ((this._data[offset + 1] << 16) | (this._data[offset + 2] << 8) | this._data[offset + 3]);
    }
    readInt8(offset = 0) { const v = this._data[offset]; return v > 127 ? v - 256 : v; }
    writeUInt8(value, offset = 0) { this._data[offset] = value & 0xff; return offset + 1; }
    ...
    toString(encoding = 'utf8', start = 0, end = this.length) {
      ...
      const sub = this._data.subarray(start, end);
      if (enc === 'latin1' || enc === 'binary' || enc === 'ascii') {
        let s = '';
        for (let i = 0; i < sub.length; i++) s += String.fromCharCode(sub[i]);
        return s;
      }
      if (enc === 'base64') return base64Encode(sub);
      if (enc === 'hex') {
        let h = '';
        for (let i = 0; i < sub.length; i++) h += (sub[i] < 16 ? '0' : '') + sub[i].toString(16);
        return h;
      }
