...
```bash
ELYXION_VERSION=v1.3.3 curl -fsSL https://raw.githubusercontent.com/xyz-elyxion/elyxion-cli/main/scripts/install.sh | bash
```

The installer defaults to a pinned, known-good release (`v1.3.3`) instead of `latest`, so a stale or broken tagged binary is never silently installed.
